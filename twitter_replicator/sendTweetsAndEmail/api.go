...
func follow(res http.ResponseWriter, req *http.Request, params httprouter.Params) {
	ctx := appengine.NewContext(req)
	...
	followeeKey := datastore.NewKey(ctx, "Follows", params.ByName("user"), 0, followerKey)
	...
	// put in datastore
	_, err = datastore.Put(ctx, followeeKey, &F{params.ByName("user")})
	if err != nil {
	...
	emailKey := datastore.NewKey(ctx, "Users", params.ByName("user"), 0, nil)
	var u User
	...
	http.Redirect(res, req, "/user/"+ params.ByName("user"), 302)
}

func unfollow(res http.ResponseWriter, req *http.Request, params httprouter.Params) {
	ctx := appengine.NewContext(req)
	// get session
	memItem, err := getCachedUser(req)
	if err != nil {
